  ...
  const fetchAll = async () => {
    try {
      ...
      if (
        ...
        productsRes.errorMessage ||
        usersRes.errorMessage ||
        salesRes.errorMessage ||
        entriesRes.errorMessage ||
        quotationsRes.errorMessage ||
        invoicesRes.errorMessage ||
        branchesRes.errorMessage ||
        warehouseLocationsRes.errorMessage ||
        clientsRes.errorMessage ||
        brandingRes.errorMessage ||
        manufacturersRes.errorMessage
      ) {
        const firstError = [
          categoriesRes,
          productsRes,
          ...
          brandingRes,
          manufacturersRes,
        ].find((r) => r.errorMessage);
        throw new Error(
          firstError?.errorMessage || "Failed to load inventory data",
        );
      }
      ...
      setEmployees(
        (usersRes.data?.data || []).filter((user: Employee) =>
      ...
      setBranches(
        (branchesRes.data?.data || []).filter((branch: any) => branch.isActive),
      );
      setWarehouseLocations(warehouseLocationsRes.data?.data || []);
      setClients(
        (clientsRes.data?.data || []).filter(
